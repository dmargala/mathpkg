...
exportToCamb[filename_,cosmology_,OptionsPattern[]]:=With[{
    ...
    redshifts=OptionValue["redshifts"],
...
"num_redshifts = "<>ToString[Length[redshifts],form]
};
lines=Join[lines,
  Flatten[Table[{
    "transfer_redshift("<>ToString[i]<>") = "<>ToString[redshifts[[i]],form],
    "transfer_filename("<>ToString[i]<>") = transfer_"<>ToString[i]<>".dat",
    "transfer_filename("<>ToString[i]<>") = matterpower_"<>ToString[i]<>".dat"
  },{i,Length[redshifts]}]]
];
Export[filename,Append[lines,""],"Table"]
]]
Options[exportToCamb]={"form"->CForm,"kmax"->2,"kPerLogInt"->0,"redshifts"->{0}};
